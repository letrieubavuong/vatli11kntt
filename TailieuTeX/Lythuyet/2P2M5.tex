...
\begin{center}
\begin{tikzpicture}[draw=Mapcolor, very thick,scale=0.9,transform shape,x=(0:1.2), y=(90:1.0), z=(-150:1.0),
...
\angle=\k*90/(\nVectorsPerNode+1);}]
in {1,...,\nVectorsPerNode}{
\draw[vector,Mapcolor!50] (\iOffset*pi/2+\t,0,0) -- ++(0,{\A*sin(2*\angle+\iOffset*180)},0);
}
}
\def\drawBNode{ % draw B node and vectors with some offset
\draw[Mapcolor!40,very thick,variable=\t,domain=\iOffset*pi/2:(\iOffset+1)*pi/2*1.01,samples=40]
plot (\t,0,{\A*sin(\t*360/pi)});
\foreach \k [evaluate={\t=\k*pi/2/(\nVectorsPerNode+1);
\angle=\k*90/(\nVectorsPerNode+1);}]
in {1,...,\nVectorsPerNode}{
\draw[vector,Mapcolor!30] (\iOffset*pi/2+\t,0,0) -- ++(0,0,{\A*sin(2*\angle+\iOffset*180)});
}
}
% main axes
