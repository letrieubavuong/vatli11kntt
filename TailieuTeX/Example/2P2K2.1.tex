\begin{vidu}%CTST
Một sóng truyền trên một dây rất dài có phương trình: $u=10\cos(2\pi t+0,01\pi x)$. Trong đó u và x được tính bằng cm và t được đo bằng giây. Hãy xác định:
\begin{itemize}
\item[a)] Chu kì, tần số và biên độ sóng. 
\item[b)] Bước sóng và tốc độ truyền sóng.
\item[c)] Giá trị của li độ u của điểm có vị trí $x=50\ cm$ ở thời điểm $t=4\ s$. 
\end{itemize}
\loigiai{
\begin{itemize}
...
\begin{itemize}
\item[a)] Độ lệch pha giữa hai điểm trên cùng phương truyền sóng, cách nhau 360 cm tại cùng một thời điểm là ..............
\item[b)] Độ lệch pha tại cùng một điểm trên phương truyền sóng sau một khoảng thời gian là 0,1 s  là ..............
\end{itemize}
\loigiai{
\begin{itemize}
...
\begin{center} 
\begin{tikzpicture}[scale=0.8,thick,>=stealth',transform shape]
%Hệ trục toạ độ
...
\draw[blue,very thick,->](6,-1)--node[midway,below,black]{Phương truyền sóng}(9,-1);
\end{tikzpicture}
\end{center} 
\begin{itemize}
\item[a)] Xác định biên độ và bước sóng của sóng.
\item[b)] Nếu chu kì của sóng là 1 s thì tần số và tốc độ truyền sóng bằng bao nhiêu?
\item[c)] Bước sóng sẽ bằng bao nhiêu nếu tần số tăng lên 5 Hz và tốc độ truyền sóng không đổi? 
\end{itemize}
\loigiai{
\begin{itemize}
...
\immini[thm]{Một sóng cơ truyền trên mặt nước với tần số $f = 10$ Hz, tại một thời điểm nào đó các phần tử mặt nước có dạng như hình vẽ. Trong đó khoảng cách từ vị trí cân bằng của A đến vị trí cân bằng của D là 30 cm và điểm C đang từ vị trí cân bằng của nó đi xuống. Chiều truyền và vận tốc truyền sóng là}
{\begin{tikzpicture}[scale=1,thick,>=stealth']
...
\choice
{\begin{tikzpicture}[scale=1,thick,>=stealth',x=0.5cm,y=0.5cm]
\draw[->] (0,0)--(11,0)node[above]{$x\ (m)$};
\draw[->] (0,-2.5)--(0,2.5)node[right]{$u\ (cm)$};
\draw[dashed] (0,-2)--(10,-2)(0,2)--(10,2)(5,0)--(5,2);
\draw [red, line width = 1.2pt, domain=0:7.5, samples=500]%
...
\draw[fill=black] (0,-2) circle (1.2 pt);
\end{tikzpicture}}
{\True \begin{tikzpicture}[scale=1,thick,>=stealth',x=0.5cm,y=0.5cm]
\draw[->] (0,0)--(11,0)node[above]{$x\ (m)$};
\draw[->] (0,-2.5)--(0,2.5)node[right]{$u\ (cm)$};
\draw[dashed] (0,-2)--(10,-2)(0,2)--(10,2)(5,0)--(5,2)(10,0)--(10,-2);
\draw [red, line width = 1.2pt, domain=0:10, samples=500]%
...
\addplot[line width=1.2pt,domain=0:6,samples=200,red,xshift=-0.45cm]{1.5*cos(deg(0.5*pi*x))};
...
\addplot[line width=1.2pt,domain=0:6,samples=200,red]{1.5*cos(deg(0.5*pi*x-pi/4))};
...
\begin{vidu}%[3P2-4.1G][Loai1]
\immini[thm]{\nguon{MH - 2017} Một sóng ngang hình sin truyền trên một sợi dây dài. Hình vẽ bên là hình dạng của một đoạn dây tại một thời điểm xác định. Trong quá trình lan truyền sóng, khoảng cách lớn nhất giữa hai phần tử M và N có giá trị  \textbf{gần nhất} với giá trị nào sau đây?}{\begin{tikzpicture}[scale=1,thick,>=stealth']
\pgfplotsset{width=7.5cm,height=4cm,compat=1.4}
\begin{axis}[
axis lines=middle,
...
\addplot[line width=1.2pt,domain=0:8,samples=200,red]{1.5*cos(deg(0.25*pi*x-pi/2))};
